...
const post ={
    ...
    async updatePermission(id,object){
        try{
            const result = await query.query("update user_permission set permission_id=? where user_id=?",[object.permission,id])
            return result;
        }catch(error){
            console.log(error)
            return error
    ...
    async deletePermission(id){
            const post = await query.query("delete from user_permission where permission_id=?",[id])
                .catch((err)=>{
                    throw err;
                })
            return post;
